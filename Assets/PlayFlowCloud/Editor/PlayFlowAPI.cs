...
public class PlayFlowAPI
{
    ...
    public static string Upload(string fileLocation, string token,string region, string servertag)
    {
        ...
        try
        {
            ...
            EditorUtility.DisplayProgressBar("Uploading to PlayFlow", "Uploading file to server", 0.75f);
    ...
    public static async Task<string> RestartServer(string token,string region, string arguments, string ssl, string match, bool update, string server_tag = "default")
    {
        string output = "";
        try
        {
        ...
        using (var formData = new MultipartFormDataContent())
        {
            ...
            formData.Headers.Add("update", update.ToString());
            formData.Headers.Add("server-tag", server_tag);
